...
/**Pour afficher les errors IMPORTANT POUR DEBUG */
...
class DB {
    ...
    private function __construct() {
        $dbname = "dt213544"; //A MODIFIER! (nom de la base Postgres)
        $host   = "woody";
        $user   ="dt213544"; //A MODIFIER ! (login compte Postgres)
        $pwd    = "29082001";   //A MODIFIER ! (mot de passe compte Postgres)
        $port   = 5432;
        ...
        try {
            // Connexion à la base
            $this->connect = new PDO($connStr, 'dt213544', '29082001'); //A MODIFIER !
    ...
    private function execQuery($requete,$tparam,$nomClasse) {
        ...
        //on indique que l'on va récupére les tuples sous forme d'objets instance de Message
    ...
    public function getMessages() {
        ...
        return $this->execQuery($requete,null,'Message');
    }



    public function insertMessage($titre, $auteur,$email, $datep, $texte) {
        $requete = 'insert into Messages values(?,?,?,?,?)';
        $tparam = array($titre, $auteur,$email, $datep, $texte);
